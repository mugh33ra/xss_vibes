...
class Main:
    ...
    def validator(self, arr, param_name, url):
        ...
        payload = '<i>msec'
        try:
            final_parameters = self.parser(url, param_name, payload)
            parsed_url = urlparse(url)
            new_url = parsed_url.scheme + "://" + parsed_url.hostname + parsed_url.path
            if self.headers:
            ...
            if payload in response:
                print(Fore.GREEN + f"[+] {payload} is reflecting in the response")
                dic[param_name].append(payload)
            elif payload.lower() in response.lower():
                print(Fore.YELLOW + f"[+] {payload} is reflected but possibly encoded or altered")
            else:
                print(Fore.RED + f"[-] {payload} is NOT reflecting in the response")
    ...
    def fuzzer(self, url):
        ...
        for parameter in parameters:
            ...
            # Instead of calling validator with dangerous characters, just test the simple payload
            data.append({parameter: ['<i>msec']})
        if not threads or threads == 1:
            print("[+] PAYLOAD TESTING HAS BEEN COMPLETED")
        return data
    ...
    def scanner(self,url):
        ...
        for data in out:
            for key in data:
                # Instead of filtering payloads from payloads.json, use a fixed simple payload
                payload_list = ['<i>msec']
